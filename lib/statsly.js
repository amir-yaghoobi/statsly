...
class Statsly {
  ...
  increment(category, statsKey, value = 1) {
    debug('incrementing category: "%s" key: "%s" by "%d"', category, statsKey, value)
    const key = this.__getStatsKey(category, this.__dateToString(new Date()))
    return Promise.fromCallback(cb => {
      this.__getPipeline().hincrby(key, statsKey, value, cb)
      this.__getPipeline().expire(key, this.defaultExpiry)
    })
  }

  set(category, statsKey, value) {
    debug('set category: "%s" key: "%s" to "%d"', category, statsKey, value)
    const key = this.__getStatsKey(category, this.__dateToString(new Date()))
    return Promise.fromCallback(cb => {
      this.__getPipeline().hset(key, statsKey, value, cb)
      this.__getPipeline().expire(key, this.defaultExpiry)
    })
  }

  reset(category) {
    debug('resetting category: "%s"', category)
    const key = this.__getStatsKey(category, this.__dateToString(new Date()))
